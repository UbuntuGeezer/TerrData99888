-- * CheckParcel.psq/sql - Check Terrxxx_SC.db for parcel ID present.
...
.open '$pathbase/$scpath/Terr258/Terr258_SC.db' 
...
UPDATE counter
SET nrecs =
CASE WHEN (SELECT COUNT() OwningParcel FROM Terr258_SCBridge
...
.separator " "
select 'Terr258_SC.db', propid, case when nrecs > 0 then 'exists' else 'does not exist' end
