...
# 2023-11-25.	wmk.	(automated) Version 3.0.6 paths eliminated (HPPavilion).
# UnKillTerrMakes.sh - UnKill *make files within territory xxx.
# 9/12/23.	wmk.
#
# Usage. bash  UnKillTerrMakes.sh <terrid> [ru|sc]
#
...
P2=${2,,}
if [ -z "$P1" ];then
 printf "%s\n" "UnKillTerrMakes <terrid> [ru|sc] missing parameter(s) - abandoned."
 exit 1
fi
case $P2 in
 "ru")
 ...
 ;;
 *)
  printf "%s\n" "UnKillTerrMakes <terrid> [ru|sc] unrecognized '$P2' - abandoned."
...
if ! test -s $TEMP_PATH/MakeFiles.txt;then
 printf "%s\n" "  UnKillTerrMakes - no *make files to process in Terr$P1.."
 read -p "Enter ctrl-c to remain in Terminal: "
 exit 0
fi
...
while read -e;do
 fn=$REPLY
 printf "%s\n" "  processing $fn..."
 $bashpath/UnKillMake.sh $fn ./
#
done < $file
popd > /dev/null
#endprocbody
printf "%s\n" "  UnKillTerrMakes $P1 $P2 complete."
~/sysprocs/LOGMSG "  UnKillTerrMakes $P1 $p2 complete."
# end UnKillTerrMakes.sh
