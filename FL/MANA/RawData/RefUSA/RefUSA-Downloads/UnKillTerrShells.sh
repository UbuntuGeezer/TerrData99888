...
# 2023-11-25.	wmk.	(automated) Version 3.0.6 paths eliminated (HPPavilion).
# UnKillTerrShells.sh - UnKill .sh files within territory xxx.
# 9/12/23.	wmk.
#
# Usage. bash  UnKillTerrShells.sh <terrid> [ru|sc]
#
...
# 11/25/23.	wmk.	(automated) Version 3.0.6 Make old paths removed.
# 9/12/23.	wmk.	original shell; adapted from UnKillTerrMakes.
#
# Notes. 
#
# P1=<terrid>
...
P1=$1
if [ -z "$P1" ];then
 printf "%s\n" "UnKillTerrShells <terrid> [ru|sc] missing parameter(s) - abandoned."
 exit 1
fi
#	Environment vars:
if [ -z "$TODAY" ];then
...
if ! test -s $TEMP_PATH/ShellFiles.txt;then
 printf "%s\n" "  UnKillTerrShells - no .sh files to process in Terr$P1.."
...
while read -e;do
 fn=$REPLY
 printf "%s\n" "  processing $fn..."
 $bashpath/UnKillShell.sh $fn ./
#
done < $file
popd > /dev/null
#endprocbody
printf "%s\n" "  UnKillTerrShells $P1 $P2 complete."
~/sysprocs/LOGMSG "  UnKillTerrShells $P1 $p2 complete."
# end UnKillTerrShells.sh
