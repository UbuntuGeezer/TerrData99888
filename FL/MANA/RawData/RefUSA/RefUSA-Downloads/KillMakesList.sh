...
# 2023-11-25.	wmk.	(automated) Version 3.0.6 paths eliminated (HPPavilion).
# KillMakesList.sh - Kill *make files using TerrMakesList.txt.
# 9/12/23.	wmk.
#
# Usage. bash  KillMakesList.sh [ru|sc]
#
...
case $P1 in
 ...
 ;;
 *)
  printf "%s\n" "KillMakesList [ru|sc] unrecognized '$P1' - abandoned."
...
while read -e;do
 ...
 printf "%s\n" "  processing $fn..."
 tn=${fn:4:99}
 $projpath/KillTerrMakes.sh $tn $P1
 cntprocessed=$((cntprocessed+1))
#
done < $file
printf "%s\n" "  KillMakesList $cntprocessed folders processed."
popd > /dev/null
#endprocbody
printf "%s\n" "  KillMakesList $P1 $P2 complete."
~/sysprocs/LOGMSG "  KillMakesList $P1 $p2 complete."
# end KillMakesList.sh
